...
#### Hash（字典）
value内部结构为一个HashMap，特别适合用来存储对象
应用场景：可以用来存储用户信息、商品信息、订单信息等。

#### Set（集合）
无序数据集合，类似List的一个列表，不能有重复数据。
应用场景：取交集、并集、差集。因此可以用来求共同好友、共同兴趣等。
...
## 热点key有哪些解决方案
### 缓存于客户端本地
将热点key对应value缓存在客户端本地，并且设置一个失效时间。对于每次读请求，将首先检查key是否存在于本地缓存中，如果存在则直接返回，如果不存在再去访问分布式缓存的机器。

### 设置永不过期
从redis上看，不设置过期时间，这就保证了，不会出现热点key过期问题，也就是“物理”不过期。我们把过期时间存在key对应的value里，如果发现要过期了，通过一个后台的异步线程进行缓存的构建，也就是“逻辑”过期
...
## redis的并发竞争是什么？如何解决？
所谓的并发竞争，是说多个客户端同时并发写一个key，可能本来应该先到的数据后到了，导致数据错乱。
...
## redis为什么这么快
1. 完全基于内存操作
2. 数据结构简单，类似hashmap， 查找的时间复杂度O（1）
3. 采用单线程，避免了不必要的上下文切换，不存在加锁释放锁造成的性能消耗
4. 使用IO多路复用，非阻塞IO（多路指的是多个网络连接，复用指的是复用同一个线程）